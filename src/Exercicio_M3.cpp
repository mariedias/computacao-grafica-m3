 ...
 * - Leitura do nome da textura (map_Kd) do arquivo .MTL;
 * - Carregamento da imagem e desenho dos objetos texturizados;
 *
 * Teclas utilizadas:
...
/*
 * Le um arquivo .OBJ: monta o buffer de vertices (posicao + coordenada de
 * textura) e envia para a GPU. Descobre a textura pelo mtllib -> map_Kd.
 *
 * Retorna o VAO, ou -1 se nao conseguir abrir o arquivo.
 * nVertices e caminhoTextura sao preenchidos por referencia
 * (caminhoTextura fica vazio se o modelo nao tiver textura).
 */
int loadSimpleOBJ(string filePath, int &nVertices, string &caminhoTextura)
{
    ...
    while (getline(arqEntrada, line))
    {
        ...
        else if (word == "f")
        {
            ...
            while (ssline >> word)
            {
                ...
                if ((vi >= 0 && vi < (int)vertices.size()) && (ti >= 0 && ti < (int)texCoords.size()))
                {
                    faceIndices.push_back(glm::ivec2(vi, ti));
                }
            }
            ...
            {
                glm::ivec2 tri[3] = {faceIndices[0], faceIndices[i], faceIndices[i + 1]};
                for (int k = 0; k < 3; k++)
                {
    ...
    if (!nomeMTL.empty()) 
    {
        ...
        string nomeTextura = loadMTL(caminhoMTL);

        if (!nomeTextura.empty())
            caminhoTextura = diretorio + nomeTextura;
    }
...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Exercício M3", nullptr, nullptr);
    if (!window)
    {
    ...
    GLuint suzanneVAO = loadSimpleOBJ("../assets/Modelos3D/Suzanne.obj", nVertices, caminhoTextura);
    ...
    if (!caminhoTextura.empty())
    {
        ...
        texID = loadTexture(caminhoTextura, imgW, imgH);
    }
